...
if ($num_replicates == 3) {
    ($CLIP{"_01"}{fi},$del) = split(/\|/,$labels[0]);
    ($CLIP{"_02"}{fi},$del) = split(/\|/,$labels[8]);
    ($CLIP{"input"}{fi},$del) = split(/\|/,$labels[16]);
    $labels[0] = "CDS";
    $labels[8] = "CDS";
    $labels[16] = "CDS";
    for (my $i=0;$i<8;$i++) {
    ...
    for(my $i=8;$i<16;$i++) {
	$labels[$i] = $UID."_02|".$labels[$i];
    }
    $type_flag = "two_replicate_ENCODEstyle";
    @rep_listing = ("_01","_02");

    
} elsif ($num_replicates == 2) {
    ($CLIP{"_01"}{fi},$del) = split(/\|/,$labels[0]);
